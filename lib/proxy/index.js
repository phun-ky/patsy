...
var _okString = isWin ? ' v '.bold.green : ' ✓ '.green;
var _failString = isWin ? ' x '.bold.red : ' ✗ '.red;
            ...
            if(matchedPath && matchedPath[0] !== '/'){
              ...
              route_opts =  {
                route : {
                  ...
                  port: parsed_resource_pass.port || 80
                },
        ...
        var proxyServer = httpProxy.createServer(function (req, res, proxy) {
          ...
          if(_opts && gotMatch){
            ...
            try{
              ...
              util.print("FAILED\n".red);
              util.puts('>>'.red + ' Could not proxy given URL: ' + req.url, error);
            ...
            try{
              ...
              if(opts.verbose){
                ...
                util.print("WARN".yellow + ' No match found, relaying to: ' + cfg.project.environment.host +':'+cfg.project.environment.port + '...' );
