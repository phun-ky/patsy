...
module.exports = function(opts){
  ...
  return {
    ...
    start : function(){
      ...
      this.stdin.on('data', this.checkInput);
    ...
    loadStaticFileServer : function(config){
      /*if(opts.verbose){
        util.print("loadStaticFileserver: Starting server...");
      }

      this.static_server  = this.proxy.startStaticFileServer(config.project.environment.webroot || "/");*/
    },
    // The purpose of this method is to load the proxy
    loadProxy : function(config){

      if(opts.verbose){
        util.puts('[Proxy]:'.magenta + ' Loading server...');
        //this.proxy.verbose = true;
      }

      // Set options to pass to proxy
    ...
    loadGrunt : function(config){
      ...
      if(process.platform == 'win32'){
        ...
        var _newArgs   = ["/c", _cmdName].concat(['--path', config.project.environment.abs_path].concat(opts.verbose ? ['-v'] : ['']));
        ...
        this.grunt       = spawn('node_modules/.bin/grunt',['--path', config.project.environment.abs_path].concat(opts.verbose ? ['-v'] : ['']));
      }
