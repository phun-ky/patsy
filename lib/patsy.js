...
process.on('uncaughtException', function (error) {
  if(String(error.code) == 'EACCES'){
   ...
   console.log('>> EXCEPTION'.red + ':' + error.code);
  }
  process.exit(1);
});
  ...
  return {
    ...
    loadGrunt : function(config){
      ...
      if(process.platform == 'win32'){
        ...
        _newArgs   = ["/c", _cmdName, '--path', config.project.environment.abs_path, '--force'];
      ...
      this.grunt.stderr.on('data',function(data){
        ...
        util.print("[Patsy]: The grunt is sick my Liege! ".yellow + ': ' +  data);
      });
