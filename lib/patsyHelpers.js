...
patsyHelpers.checkInput = function(chunk) {
  ...
  if(chunk == 'exit'){
    ...
    util.print("[Patsy]: The only way to run tests for now is by toggling the build.options.testsOnWatch to true!\n".yellow);
...
patsyHelpers.runGruntTasks = function(how){
  var exec = require('child_process').exec,
    ...
    var config    = patsyHelpers.loadPatsyConfigInCurrentProject();
console.log(config);
    process.chdir(patsyHelpers.appPath);
    ...
    if(process.platform == 'win32'){
      ...
      execCmd     = 'cmd /c node_modules\\.bin\\grunt --path ' + config.project.path + ' ' + how;

    } else {

      execCmd       = 'node_modules/.bin/grunt --path ' + config.project.path + ' ' + how;
    }
