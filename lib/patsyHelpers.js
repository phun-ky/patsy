...
patsyHelpers.checkProject = function(){
  ...
  try {
    ...
    if(typeof _projectConfig !== 'undefined'){
      ...
      stdout.write('[Patsy]: Follow the path of "'.yellow + _projectConfig.projectPath.yellow + '" to complete the quest of.. \n'.yellow);
      stdout.write('[Patsy]: Of.. ehm, of '.yellow + _projectConfig.nameOfProject.yellow + '"! \n'.yellow );      
...
patsyHelpers.loadStage = function(config){
  ...
  if(process.platform == 'win32'){
    ...
    var _newArgs   = ["/c", _cmdName].concat(['--path', config.projectPath, '--project',config.nameOfProject,'jshint']);  

    
    var _grunt     = spawn(_bin, _newArgs);
    
  } else {
    var _grunt       = spawn('grunt',['--path', config.projectPath, '--project',config.nameOfProject]); 
  }
