...
patsyHelpers.checkInput = function(chunk) {
  ...
  if(chunk == 'exit'){
    ...
    stdout.write("[King Arthur]: Oh very well then, Patsy, let us proceed with our quest!".yellow);
  }
}
...
patsyHelpers.checkProject = function(){
  ...
  try {
    ...
    var _projectPath      = require('path').resolve(require('path').dirname('patsy.JSON'));

    if(typeof _projectConfig !== 'undefined'){

      _projectConfig = patsyHelpers.extend(_projectConfig,{
        projectPath         : _projectPath,
        projectWebRoot      : typeof _projectConfig.webroot !== 'undefined' ? _projectPath + _projectConfig.webroot : _projectPath + '/',
        projectFolderName   : require('path').basename(_projectPath)
      });     
