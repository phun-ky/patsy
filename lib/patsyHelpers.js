 ...
 * @var     Object
 */
var Validator     = require('../node_modules/jsonschema/lib/validator');
...
patsyHelpers.validateJSON = function(schema, instance){

  // Default values added for testing
  instance = instance || 4;
...
patsyHelpers.loadPatsyConfigInCurrentProject = function(projectPath){
  ...
  if(typeof projectPath === 'undefined'){
    ...
    if(program.verbose){
      ...
      util.print('>> NOTICE: '.yellow + ' Loading project configuration: No project path set, declaring empty path.'.white + '\n');
    }
    projectPath = '';
  }
  ...
  if(fs.existsSync(_fullConfigPath)){
    ...
    var _config_json        = fs.readFileSync(projectPath + 'patsy.json', 'utf8');
    var _config_json_schema = fs.readFileSync(patsyHelpers.appPath + 'lib/json/patsy.schema.json', 'utf8');

    if(patsyHelpers.validateJSON(_config_json_schema, _config_json)){
      patsyHelpers.config = JSON.parse(_config_json);
    } else {
      throw new Error('Your patsy configuration is not a valid JSON file!');
    }
...
patsyHelpers.setup = function(){
  ...
  if(typeof patsyHelpers.config === 'undefined'){
    ...
    if(program.verbose){
      ...
      util.print('>>'.green + ' Setup: Configuration is defined, continuing..'.white + '\n');
    }
  }
  ...
  if(typeof patsyHelpers.config.project.options !== 'undefined' && patsyHelpers.config.project.options.watch_config){
    ...
    if(program.verbose){
      ...
      util.print('Setup: Not watching project configuration file.\n');
    }
  }
